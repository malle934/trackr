...
def upsert_from_sync(parsed: dict) -> tuple[Application, bool]:
    """Insert if not duplicate (same company+title), else return existing. Returns (app, is_new)."""
    apps = _read_apps()
    key = (parsed.get("company", "").lower(), parsed.get("title", "").lower())
    for a in apps:
        if (a.get("company", "").lower(), a.get("title", "").lower()) == key:
